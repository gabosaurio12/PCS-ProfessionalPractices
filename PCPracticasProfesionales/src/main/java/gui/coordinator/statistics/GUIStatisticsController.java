package gui.coordinator.statistics;
...
public class GUIStatisticsController {
    ...
    @FXML private TableColumn<Presentation, Double> averageColumn;
    ...
    public void setLabels(Student student) {
        ...
        if (!progress.isNull()) {
            ...
            utils.createAlert("Aviso",
                    "No se encontró el progreso del Estudiante, " +
                            "comunicarse son servicio técnico por favor");
        }
    ...
    public void setPresentationTable(Student student) {
        ...
        averageColumn.setCellValueFactory(cellData -> {
            ...
            try {
                evaluations = dao.getEvaluationsFromPresentation(cellData.getValue().getPresentationID());
                if (evaluations.isEmpty()) {
                    return new SimpleDoubleProperty(0.0).asObject();
                }
                double gradeSum = evaluations.stream().mapToDouble(Evaluation::getAverageGrade).sum();
                double gradeAverage = gradeSum / evaluations.size();

                return new SimpleDoubleProperty(gradeAverage).asObject();

            } catch (SQLException e) {
                utils.createAlert("Error",
                        "Hubo un error al recuperar las evaluaciones, " +
                                "No se mostrarán las calificaciones");
                logger.error("Error al recuperar evaluaciones", e);
                return new SimpleDoubleProperty(0.0).asObject();
            }
        });
    ...
    public void viewPresentationDetails() {
        ...
        if (presentation == null) {
            ...
            GUIPresentationDetailsController controller =
                    utils.openWindow(
                            "/fxml/Coordinator/Statistics/GUIPresentationDetails.fxml",
