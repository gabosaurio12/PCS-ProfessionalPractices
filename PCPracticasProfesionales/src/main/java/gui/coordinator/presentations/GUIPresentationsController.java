...
public class GUIPresentationsController {
    ...
    public void openCreatePresentation() {
        GUICreatePresentationController controller = utils.openWindow(
                "/fxml/Coordinator/Presentation/GUICreatePresentation.fxml",
                "Crear Presentación",
                GUICreatePresentationController.class);
        controller.init();
        refreshTable();
    }

    public void openEditPresentation() {
        Presentation selectedPresentation = presentationsTable.getSelectionModel().getSelectedItem();
        if (selectedPresentation != null) {
            GUIEditPresentationController controller = utils.openWindow(
                    "/fxml/Coordinator/Presentation/GUIEditPresentation.fxml",
                    "Editar Presentación",
                    GUIEditPresentationController.class);
            controller.init(selectedPresentation);
            refreshTable();
        } else {
            utils.createAlert("Aviso",
                    "Seleccione una presentación por favor");
        }
    }
