...
public class GUIStatisticsController {
    ...
    public void setLabels(Student student) {
        studentLabel.setText(student.getName().concat(" ").concat(
                student.getFirstSurname()).concat(" "));
        StudentProgress progress = new StudentProgress();
        try {
            progress = new StudentDAOImplementation().getStudentProgressByTuition(
                    student.getTuition());
        } catch (SQLException e) {
            logger.error("Error al recuperar progreso del estudiante", e);
            utils.createAlert("Error",
                    "Error al recuperar progreso del estudiante");
        }
        if (progress.isNull()) {
            utils.createAlert("Aviso",
                    "No se encontró el progreso del Estudiante, contacte a servicio técnico por favor");
        } else {
    ...
    public void viewPresentationDetails() {
        ...
        if (presentation == null) {
            ...
            GUIPresentationDetailsController controller =
                    utils.openWindow(
                            "/fxml/Student/Statistics/GUIPresentationDetails.fxml",
    ...
    public void closeDetails() {
        utils.openWindow(
                ...
                "Estudiantes",
                GUIMainPageStudentController.class, blueFringe);
    }
}
